...
// require() (não fs) de propósito: garante que o arquivo entra no bundle da
// função na Vercel. É a lista usada quando o Blob Store ainda não tem nada
// salvo (projeto novo) — sem isso, o seletor de categoria no painel
// começaria vazio até alguém criar cada categoria manualmente.
const DEFAULT_CATEGORIES = require("../data/categorias.json");
...
async function readBlobJSON(key, fallback) {
  ...
  // O Blob Store deste projeto foi criado como privado — os blobs só são
  // lidos por aqui mesmo (servidor), com o BLOB_READ_WRITE_TOKEN, nunca
  // direto pelo navegador, então não precisa (nem pode) ser "public".
...
async function listCategories() {
  const categorias = await readData(CATEGORIES_BLOB_KEY, CATEGORIES_PATH, DEFAULT_CATEGORIES);
  return categorias.length ? categorias : DEFAULT_CATEGORIES;
}
